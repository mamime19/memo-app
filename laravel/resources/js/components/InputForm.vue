...
const submit = async () => {
    try {
        const response = await axios.post('http://localhost:48080/api/memopads',{
            title: title.value
        })
        ...
        <div class="w-[600px] bg-white m-10 p-10 rounded-xl shadow-md">
            <div class="flex flex-col items-center gap-10">
                <div class="flex flex-col items-center gap-2">
                    <div class="text-3xl">5秒でメモ帳を作成をしよう！</div>
                    <div class="text-md">かんたんメモはログインなしですぐにメモ帳を作成できます</div>
                </div>
                <div class="w-full">
                ...
                <button type="button" @click="submit" class="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 me-2 mb-2 focus:outline-none">メモ帳を新規作成</button>
            </div>
        </div>
    </div>
</template>
